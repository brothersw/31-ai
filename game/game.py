...
class Game:
    def __init__(self, players: list[Agent]):
        ...
        # TODO: setup discard pile with 1 card at the start
        # TODO: setup cycling the player order
    ...
    def _next_turn(self):
        assert self.state.turn >= 0
        
        self.state.turn += 1
        self.state.turn %= len(self.players)


# TODO: rework since hands were decoupled from agents
    ...
    def end_game(self):
        ...
        for loser in losers:
            if loser == self.state.players[self.state.called]:
                loser.lives -= 2
            else:
                loser.lives -= 1
